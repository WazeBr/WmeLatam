...
function genericCacaNovato() {
    ...
    for (var seg in wazeModel.segments.objects) {
        ...
        if (Math.floor((new Date().getTime() - segmento.attributes.updatedOn) / 86400000) < 90) //Só quem editou nos ultimos 90 dias.
        {
            ...
            //funChampCapturaNovatos_Passo0(usuario.userName);
        }
    }
...
funChampCapturaNovatos_Passo1 = function (dashboardUser) {
    ...
    var b = dashboardUser.responseText.substring(dashboardUser.responseText.indexOf(" W.EditorProfile.data"));
    var usuario = JSON.parse(b.substring(36, b.indexOf(")") - 1));
...
function genericMontarHTML() {
    ...
    abaURS.innerHTML += '<p><span class="Teste"><input type="button" id="btnSeguirTodas" value="Seguir todas as URs visíveis"/></p>';
    ...
    abaURS.innerHTML += 'Mensagem:<br/>'; //@JuniorDummer - Melhoria visual ////////////////////////FALTA TRADUÇÃO///////////////////////
	abaURS.innerHTML += '<input type="text" id="_txtAberturaUR" value="" style="width:256px;line-height:14px;height:22px;margin-bottom:4px; margin-top:4px;"/><br/>';
    abaURS.innerHTML += '<input type="button" id="_MUV_btnAbre" value="Iniciar comunicação de URs"/>';
   ...
   /////////////////////*conteúdo aba Vias*/////////////////////
    ...
    abaVias.innerHTML += '<hr style="margin-top: 10px;margin-bottom:10px;">';
    abaVias.innerHTML += '<input type="button" id="btnProtegerMapa" value="Proteger o mapa"/><br/>';
    ...
    abaOutrasFuncoes.innerHTML += 'Radares<br/>';
    ...
    //genericObterCheckboxPersistido('chkNegoCurioso');
    ...
    document.getElementById('btnSeguirTodas').onclick = funUR_SeguirTodasVisiveis;
    ...
    //document.getElementById('chkNegoCurioso').onclick = genericPersistirCheckboxes;
    ...
    //window.setInterval(funOu_MedirUsuario, 60000);//1 minuto
        ...
        if (Waze.selectionManager.selectedItems.length > 0) {
            ...
            for (i = 0; i < Waze.selectionManager.selectedItems.length; i++) {
                for (var seg in wazeModel.segments.objects) {
                    ...
                    if (segmento.geometry.id == Waze.selectionManager.selectedItems[i].geometry.id) {
            ...
            if (segmentosSelecionados.length >= 1) {
                ...
                if (Waze.selectionManager.selectedItems.length == 2) {
                    if (segmentosSelecionados.length == Waze.selectionManager.selectedItems.length
                        &&
                        (
...
function funUR_SeguirTodasVisiveis() {
    ...
    if (confirm(msgConfirmacao)) {
        ...
        msgConfirmacao = genericTraducao('msg_SeguindoAgora');
        ...
        for (var urobj in wazeModel.updateRequestSessions.objects) {
            ...
            if (ureq.comments.length > 0 && ureq.open == true) {
                ...
                if (diasComentario >= diasConfComentario) {
                    ...
                    && ureq.comments.last().text.toUpperCase().indexOf('ENCERRAR') == -1
                    && ureq.comments.last().text.toUpperCase().indexOf('FECHAR') == -1
                    && ureq.comments.last().text.toUpperCase().indexOf('FINALIZAR') == -1
                    && ureq.comments.last().text.toUpperCase().indexOf('FECHE') == -1
                    && ureq.comments.last().text.toUpperCase().indexOf('ENCERRE') == -1
                        ...
                        if (Waze.map.getExtent().toGeometry().containsPoint(murInfo.geometry)) {
                            ...
                            if (
                                ...
                                if (ureq.comments.length != 0) {
                                    if (ureq.comments.last().text == textoAbertura) {
                                        break;
                                    }
                                }
    ...
    for (var segId in wazeModel.segments.objects) {
        ...
        if (funRevVia_VerificarSegmentoVisivel(segmento)) {
            ...
            if (segmento.attributes.lockRank != null
                ...
                instrucao = "\
                wazeModel.actionManager.actions.last().subActions[0].oldAttributes = { lockRank: " + segmento.attributes.lockRank + " };    \
                wazeModel.actionManager.actions.last().subActions[0].newAttributes = { lockRank: null };                                    \
...
function funVia_AplicarNomeAlternativo() {
    ...
    for (var i = 0; i < todosSelecionados.length; i++) {
        ...
        instrucao = "$('div[class=\"action-buttons\"]').find('button[class=\"btn btn-primary save-button\"]').click();";
        genericAssincCommand(instrucao);
...
//function funVia_EnviarComentario() {
//    //Google Script
//    //https://script.google.com/macros/d/1olt2evukJLwK4QUeAMR-OA90sd0qCrrhYJnMhHwWMdUXjrb8CocWEwDx/edit?splash=yes
//    //https://script.google.com/macros/s/AKfycbxK2XBekWH1GdTYv2XZHH9T-Q43h69YDDBj0dfU3U0/dev
//    //https://script.google.com/macros/s/AKfycbyQnUHxbhm92pQM5aWgJwpA_ZxGpJWQrlWXZghhtd_finqPy5w/exec
...
//                var sourceLink = 'https://script.google.com/macros/s/AKfycbyQnUHxbhm92pQM5aWgJwpA_ZxGpJWQrlWXZghhtd_finqPy5w/exec';
//                sourceLink += '?idSegmento=' + segmento.attributes.id;
//                sourceLink += '&login=' + wazeModel.loginManager.user.userName;
//                sourceLink += '&comentario=' + encodeURI(document.getElementById('txtHistoriaSegmento').value);

//                console.log('Enviar comentario ' + i);
//                genericGerarIframe(i, segmento.attributes.id, sourceLink);
//            }
//        }
//    }

//    document.getElementById('txtHistoriaSegmento').value = "";
//}
...
function funVia_AplicarMudancaCidade() {
    ...
    if (todosSelecionados.length > 0) {
        ...
        for (var i = 0; i < todosSelecionados.length; i++) {
            ...
            comando = "$('a[class=\"address-edit-btn\"]').click();";
            genericAssincCommand(comando);
            ...
            comando = "$('div[class=\"action-buttons\"]').find('button[class=\"btn btn-primary save-button\"]').click();";
            genericAssincCommand(comando);
...
function funVia_SelecionarSegmentosAlinhados(segmento1, segmento2) {
    ...
    if (chamadaRaiz) {
        console.log(seg_listaSegmentosSelecionados);
        Waze.selectionManager.select(seg_listaSegmentosSelecionados);
        seg_listaSegmentosSelecionados = [];
    }
}
...
funVia_SelecionarSegmentoClosure = function (idSegmento) {
    ...
    Waze.selectionManager.select(sel);
...
function funPlace_ObterPoint(geometryPlace) {
    if (geometryPlace.id.indexOf('Geometry.Point') != -1) {
    ...
    if (document.getElementById('chkPlaceUsuario').checked) {
        for (var objs in wazeModel.venues.objects) {
            ...
            if (!place.attributes.adLocked) {
                for (var i = 0; i < place.attributes.venueUpdateRequests.length; i++) {
                    ...
                    if (Waze.map.getExtent().toGeometry().containsPoint(funPlace_ObterPoint(place.geometry)) &&
        ...
        if (usuariosPUs.length == 0) //Já que perdeu tempo de caçar PU e naõ achou nenhum. Localiza os places.
        {
            for (var objs in wazeModel.venues.objects) {
                ...
                if (!place.attributes.adLocked &&
                    Waze.map.getExtent().toGeometry().containsPoint(funPlace_ObterPoint(place.geometry))) {
        ...
        if (landmark.attributes.venueUpdateRequests.length > 0
            ...
            if (!apenasUsuario ||
                ...
                for (var i = 0; i < landmark.attributes.venueUpdateRequests.length; i++) {
                    ...
                    {
                        if (valor) {
                            strComando = "$('[for=\"approved-true\"]').click();";
                            ...
                            if (landmark.attributes.venueUpdateRequests[i].attributes.updateType == "flag") {
                                ...
                                strComando = "$('[for=\"approved-false\"]').click();";
                            }
                            genericAssincCommand(strComando, true);
                        }
                        ...
                        {
                            strComando = "$('[class=\"btn btn-block btn-default next\"]').click();";
                            genericAssincCommand(strComando);
                        }
                    }
                }

                //strComando = "$('[class=\"btn btn-block btn-primary done\"]').click();";
                //genericAssincCommand(strComando);
            }
        }
    }
...
function funOu_RealcarPlaces() {
    ...
    if (document.getElementById('chkPlacesProblematicos').checked && global_FilaInstrucaoAssincrona.length == 0) {
        for (var objs in wazeModel.venues.objects) {
            ...
            if (!Waze.map.getExtent().toGeometry().containsPoint(funPlace_ObterPoint(landmark.geometry))) {
                continue;
            }
    ...
    if (document.getElementById('chkRoteamento').checked) {
        ...
        for (var seg in wazeModel.segments.objects) {
            ...
            if (segmentoTestado.attributes.routingRoadType != null) {
                $('[id="' + segmentoTestado.geometry.id + '"]').attr("stroke", "#66ff33");
                $('[id="' + segmentoTestado.geometry.id + '"]').attr("stroke-opacity", "1");
            }
        }
    }
    ...
    if (global_DesabilitaAutoSave == false) {
        ...
        if (document.getElementById('chkAutoSave').checked) {
            ...
            if ((wazeModel.actionManager.unsavedActionsNum() >= qtddSave) &&
                ...
                window.setTimeout(
                function () {
                    $("div[class='toolbar-button waze-icon-save ItemInactive']").trigger('click');
                }, 1000);
...
function funRevVia_IniciarFaxinaNew() {
...
function funRevVia_AbrirPainel(painelAberto) {
    if (!painelAberto) {
        var instrucao = "                                   \
            $('[class=\"address-edit-btn\"]').click();     \
            genericAssincBreak();";
        genericAssincCommand(instrucao, false);
    }
...
function funRevVia_CorrigirGrafiaNome(segmento, segmentoSts, NomesModificados, iStreetLocalizada, segmentoSelecionado, painelAberto) {
    ...
    segmentoSelecionado = funRevVia_SelecionarSegmento(segmento, segmentoSelecionado);
    ...
    if (iStreetLocalizada == 0) {
        ...
        instrucao = "$('input[class=\"form-control streetName\"]').attr(\"value\", \"" + NomesModificados[iStreetLocalizada] + "\");";
        genericAssincCommand(instrucao, false);
...
function funRevVia_AplicarCorrecao(painelAberto) {
    ...
    if (painelAberto) {
        ...
        var instrucao = "                                                                                       \
            $('div[class=\"action-buttons\"]').find('button[class=\"btn btn-primary save-button\"]').click();   \
            genericAssincBreak();";
        genericAssincCommand(instrucao, true);
    }
}
...
function funRevPla_ConverterResidencial(numero) {
    ...
    if (numero != null) {
        instrucao = "$('a[class=\"address-edit-btn\"]').click();";
        genericAssincCommand(instrucao, false);

        instrucao = "$('input[name=\"houseNumber\"]').attr('value', " + numero + ");";
        genericAssincCommand(instrucao, false);

        instrucao = "$('button[class=\"btn btn-primary save-button\"]').click();";
        genericAssincCommand(instrucao, true);
    }
...
funDarkSide_DestravarPlaces = function (idPlaceSaudavel, idPlaceCondenado) {
    $('[data-id="' + idPlaceSaudavel + '"]').click();

    window.setTimeout(function () {

        for (var i = 0; i < wazeModel.venues.objects[idPlaceCondenado].attributes.venueUpdateRequests.length; i++) {
            if (i % 2 == 0) {
                $('[for="approved-false"]').click();
            }
            else {
                $('[for="approved-true"]').click();
            }
        }

        for (var i = 0; i < wazeModel.venues.objects[idPlaceCondenado].attributes.venueUpdateRequests.length; i++) {
            wazeModel.actionManager.actions[i].approved = false;
            wazeModel.actionManager.actions[i].prevApproved = null;
            wazeModel.actionManager.actions[i].venue = wazeModel.venues.objects[idPlaceCondenado];
            wazeModel.actionManager.actions[i].request = wazeModel.venues.objects[idPlaceCondenado].attributes.venueUpdateRequests[i];
        }
...
bomba = function () {
    for (var seg in wazeModel.segments.objects) {
        ...
        if (segmento.attributes.fwdFlags == 1 ||
            segmento.attributes.revFlags == 1) {

            funRevVia_SelecionarSegmento(segmento);

            var instrucaoFechamento = "";
            if (segmento.attributes.fwdFlags == 1) {
                instrucaoFechamento = "$('[id=\"fwdSpeedCameraCheckbox\"]').click();";
                genericAssincCommand(instrucaoFechamento, true);
            }

            if (segmento.attributes.revFlags == 1) {
                instrucaoFechamento = "$('[id=\"revSpeedCameraCheckbox\"]').click();";
                genericAssincCommand(instrucaoFechamento, true);
            }
